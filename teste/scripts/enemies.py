...
class Rock_DragonFly:
    ...
    def update(self):
        if self.health <= 0:
            ...
            hspd = keys[pygame.K_RIGHT]-keys[pygame.K_LEFT]
            vspd = keys[pygame.K_DOWN]-keys[pygame.K_UP]

            self.pos = self.pos[0]+hspd,self.pos[1]+vspd
            #self.head_pos = self.head_pos[0]+hspd,self.head_pos[1]+vspd
            ###
            self.off_tick += 1/6
            ...
            self.head_pos =  self.head_pos[0]-(self.head_pos[0]-(self.pos[0]-(24-self.is_flipped*48)))*0.125,math.sin(self.off_tick)*1+self.head_pos[1]-(self.head_pos[1]-(self.pos[1]-3))*0.25
            self.body_pos =  self.body_pos[0]-(self.body_pos[0]-(self.pos[0]+(24-self.is_flipped*48)))*0.125,math.sin(self.off_tick)*2+self.body_pos[1]-(self.body_pos[1]-(self.pos[1]))*0.25
            self.tail_pos =  self.tail_pos[0]-(self.tail_pos[0]-(self.pos[0]+(48-self.is_flipped*96)))*0.125,math.sin(self.off_tick)*3+self.tail_pos[1]-(self.tail_pos[1]-(self.pos[1]))*0.125
